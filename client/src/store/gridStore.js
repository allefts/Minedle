...
const gridStore = create((set, get) => ({
  ...
  fromInvToGrid: (id, ingredientName, linkToImage) =>
    set((state) => ({
      currentGrid: state.currentGrid.map((currGridItem, gridIdx) => {
        // console.log("Grid Cell: ", id);
        // console.log("Inv Cell: ", currGridItem.id);
        if (parseInt(id.slice(-1)) == currGridItem.id) {
  ...
  fromInvToPopulatedCell: (id, ingredientName, linkToImage) =>
    set((state) => ({
      currentGrid: state.currentGrid.map((currGridItem, gridIdx) => {
        // console.log(id, currGridItem.id);
  ...
  fromGridToEmptyCell: (itemId, gridOverId, ingredientName, linkToImage) =>
    set((state) => ({
      currentGrid: state.currentGrid.map((currGridItem, gridIdx) => {
        const parsedItemId = parseInt(itemId.slice(-1));
        const parsedGridOverId = parseInt(gridOverId.slice(-1));

        if (gridIdx == parsedGridOverId) {
          currGridItem.empty = false;
          currGridItem.ingredientName = ingredientName;
          currGridItem.linkToImage = linkToImage;
        } else if (gridIdx == parsedItemId) {
  ...
  checkGrid: (
    ...
    if (resultItemType === "shapeless") {
      ...
      const resItems = resultItemIngredients
        .map((resItem) => {
          return resItem[0].Value.replaceAll("minecraft:", "");
        })
        .sort();
      ...
      // console.log("Result Num Rows:", numRows);
      // console.log("Result Num Cols:", numCols);
      ...
      for (const itemInGridIdx in itemsInGrid) {
        if (itemsInGrid[itemInGridIdx]) {
          for (const resultKeyItemIdx in resultItemKeys) {
            const formattedItemInGrid = itemsInGrid[itemInGridIdx]
              .replaceAll("_", " ")
              .split(" ")
              .map((el) => el[0].toUpperCase() + el.substring(1))
              .join(" ");
            ...
            if (
              formatItemName(
                resultItemKeys[resultKeyItemIdx].Value[0].Value
              ) === formattedItemInGrid
            ) {
      ...
      // console.log("Grouped Grid: ", groupedGrid);
